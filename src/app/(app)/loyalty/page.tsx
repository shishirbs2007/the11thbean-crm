...
export default async function LoyaltyPage() {
  ...
  const [accountsResult, cardsResult, ledgerResult, peopleResult] =
    await Promise.all([
      supabase
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-2">
        <section className="rounded-2xl border p-6">
          ...
          >
            ...
            <select
              aria-label="Type of loyalty transaction"
        ...
        <section className="rounded-2xl border p-6">
          ...
          >
            ...
            />
            <select
              aria-label="Choose the gift card owner"
            ...
            <select
              aria-label="Choose the purchaser"
              name="purchaser_person_id"
              className="rounded-xl border px-3 py-2"
            >
            ...
            {accounts.map((account) => {
              ...
              return (
                <Link
                  ...
                  className="grid gap-2 rounded-xl bg-neutral-50 p-4 text-sm sm:grid-cols-4"
                >
                  ...
                  <span className="sm:text-right">
                    ₹{Number(account.wallet_balance).toFixed(0)}
                  </span>
                </Link>
              );
            })}
          ...
          <div className="mt-5 space-y-3">
            {cards.map((card) => (
              ...
              >
                ...
                <span>
                  ₹{Number(card.remaining_value).toFixed(0)} remaining
                </span>
        ...
        <div className="mt-5 space-y-3">
          {ledger.map((entry) => {
            ...
            return (
              <div
                key={entry.id}
                className="grid gap-2 rounded-xl bg-neutral-50 p-4 text-sm sm:grid-cols-5"
              >
                <span>
                  {person?.preferred_name || person?.first_name || "Unknown"}{" "}
                  {person?.last_name || ""}
