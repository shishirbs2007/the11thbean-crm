...
import { searchCustomers, submitPosOrder, type CustomerMatch } from "./actions";
...
export function Register({
  ...
  const change =
    paymentMethod === "cash" && tenderedValue !== null
      ? Math.round((tenderedValue - total) * 100) / 100
      : null;
  ...
  // Debounced customer lookup (online only).
  useEffect(() => {
    const query = customerQuery.trim();
    if (query.length < 2) {
      const clear = setTimeout(() => setCustomerResults([]), 0);
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_360px]">
        ...
        <section className="rounded-2xl border p-4">
          ...
          <div className="mt-4">
            ...
            {customer ? (
              <div className="mt-1 flex items-center justify-between rounded-xl border px-3 py-2 text-sm">
                <span>
                  {customer.name}
              ...
              <div className="relative mt-1">
                ...
                {customerResults.length > 0 && (
                  <ul className="absolute z-10 mt-1 w-full overflow-hidden rounded-xl border bg-white shadow">
