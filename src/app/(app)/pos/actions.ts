...
export async function searchCustomers(query: string): Promise<CustomerMatch[]> {
  ...
  const { data } = await supabase
    ...
    .or(
      `first_name.ilike.${pattern},last_name.ilike.${pattern},preferred_name.ilike.${pattern},phone.ilike.${pattern}`,
    )
    .eq("is_active", true)
    .limit(8);
  ...
  return (data ?? []).map((p) => {
    const name =
      [p.preferred_name || p.first_name, p.last_name]
        ...
        .join(" ")
        .trim() || "Unnamed";
    return { id: p.id as string, name, phone: (p.phone as string) ?? null };
  });
}
